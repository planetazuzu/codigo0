...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@shared/schemas": path.resolve(__dirname, "./backend/src/shared/schemas/index.ts"),
  ...
  build: {
    // Evitar preload de módulos que generan advertencia "preloaded but not used"
    modulePreload: false,
    ...
    minify: 'esbuild',
    // Generar source maps para producción (útil para debugging)
    sourcemap: false, // Desactivado para reducir tamaño, activar si necesitas debugging
    // Optimizar assets
  ...
  test: {
    ...
    coverage: {
      ...
      // Objetivo 80% (TICKET-019); descomentar cuando se alcance
