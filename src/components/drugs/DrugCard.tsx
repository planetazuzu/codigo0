...
const DrugCard = memo(({ drug, defaultExpanded = false }: DrugCardProps) => {
  const [isExpanded, setIsExpanded] = useState(defaultExpanded);
  const { isFavorite, toggleFavorite: toggleFavoriteHook } = useFavorites();

  // Guard defensivo: no renderizar si drug es inválido (después de hooks)
  if (!drug || typeof drug !== 'object' || !drug.id) {
    return null;
  }
