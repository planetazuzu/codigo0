...
const ManualViewer = () => {
  ...
  // Guard clause: validar parámetros requeridos
  if (!parte || !bloque || !capitulo) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        <BookOpen className="w-12 h-12 text-muted-foreground" />
        <h2 className="text-xl font-semibold text-foreground">Parámetros inválidos</h2>
        <p className="text-muted-foreground">Faltan parámetros requeridos en la URL</p>
        <Link
          to="/manual"
  ...
  useEffect(() => {
    ...
    if (cap) {
      ...
      // Si no se encuentra, limpiar estado
      setCapituloData(null);
      setFilePath('');
    }
  }, [capitulo]);
  ...
  // Cláusula de guarda: verificar que la navegación existe antes de buscar capítulos
  const capituloAnterior = capituloData?.navegacion?.anterior
    ? getCapituloById(capituloData.navegacion.anterior) ?? null
    : null;
  const capituloSiguiente = capituloData?.navegacion?.siguiente
    ? getCapituloById(capituloData.navegacion.siguiente) ?? null
    : null;
